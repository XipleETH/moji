...
async function main() {
    console.log("🎫 COMPRANDO 50 TICKETS EN CONTRATO V5");
    ...
    const totalCost = ticketPrice * 50n;
    
    console.log("\n💰 INFORMACIÓN FINANCIERA:");
    console.log("- Balance USDC:", ethers.formatUnits(usdcBalance, 6), "USDC");
    console.log("- Costo total (50 tickets):", ethers.formatUnits(totalCost, 6), "USDC");
    ...
    // Generar 50 combinaciones aleatorias
    console.log("\n🎲 GENERANDO 50 COMBINACIONES ALEATORIAS...");
    ...
    for (let i = 0; i < Math.min(5, ticketCombinations.length); i++) {
        const combo = ticketCombinations[i];
        const emojis = combo.map(num => EMOJI_MAP[num]).join(' ');
        console.log(`  ${i + 1}. [${combo.join(', ')}] → ${emojis}`);
    }
    if (ticketCombinations.length > 5) {
        console.log(`  ... y ${ticketCombinations.length - 5} combinaciones más`);
    }
    ...
    for (let batchStart = 0; batchStart < ticketCombinations.length; batchStart += BATCH_SIZE) {
        ...
        for (let i = 0; i < currentBatch.length; i++) {
            ...
            try {
                console.log(`🎫 Comprando ticket ${ticketIndex + 1}/50: [${combination.join(', ')}]`);
                
                const buyTx = await contract.buyTicket(combination, {
                    gasLimit: 500000 // Gas limit conservador
                });
    ...
    if (successCount > 0) {
        ...
        console.log("⏰ Próximo sorteo: Cada día a las 16:00 UTC");
