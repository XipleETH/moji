...
async function runScriptParallel(scriptName, networkName, emoji, hardhatNetwork, ticketCount) {
    return new Promise((resolve, reject) => {
        console.log(`${emoji} INICIANDO ${networkName} - ${ticketCount} tickets...`);
...
async function main() {
    console.log("🚀 COMPRA PARALELA DE 200 TICKETS TOTAL");
    console.log("=".repeat(60));
    console.log("🏔️  Avalanche Fuji: 100 tickets (20 USDC)");
    console.log("🔷  Base Sepolia: 100 tickets (20 USDC)");
    console.log("💰 TOTAL: 200 tickets (40 USDC)");
    console.log("=".repeat(60));
    console.log("\n⚠️  IMPORTANTE:");
    console.log("- Asegúrate de tener al menos 20 USDC en cada red");
    ...
    // Ejecutar ambos scripts en paralelo con las redes especificadas (100 tickets cada uno)
    const [avalancheResult, baseResult] = await Promise.all([
        runScriptParallel('buy-100-tickets-avalanche-fuji.js', 'AVALANCHE FUJI', '🏔️', 'avalanche-fuji', 100),
        runScriptParallel('buy-100-tickets-base-sepolia.js', 'BASE SEPOLIA', '🔷', 'base-sepolia', 100)
    ]);
        ...
        if (result.success) {
            ...
            if (result.output) {
                ...
                if (ticketMatches) {
                    ...
                    // Si no se puede extraer del output, usar el valor esperado
                    totalTickets += result.ticketCount;
                    console.log(`   🎫 Tickets esperados: ${result.ticketCount}`);
                }
